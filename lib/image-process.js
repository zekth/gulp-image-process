...
module.exports = function(param) {
  ...
  parameters.progressive = param.progressive || false
  ...
  parameters.multipleResize = param.multipleResize || false
  ...
  let processMultipleResize = async (file, task) => {
    ...
    if (Array.isArray(parameters.multipleResize)) {
      for (let s of parameters.multipleResize) {
        verboseLog(`${chalk.yellow('Multiple Resize')} ${fileInfo.name} Size:${s}`)
  ...
  let processWatermark = async image => {
    ...
    let processWatermarkResize = watermark => {
      verboseLog(
        `${chalk.yellow('Watermark')} size: width:${watermarkMetadata.width} height:${watermarkMetadata.height}`
      )
  ...
  async function processFiles(file, enc, cb) {
    ...
    img
      .jpeg({ quality: parameters.quality, force: false, progressive: parameters.progressive })
      .png({ compressionLevel: pngCompressionLevel, force: false })
      .toBuffer()
      .then(async (data, info) => {
        file.contents = data
        if (parameters.multipleResize) {
          verboseLog(`${chalk.yellow('Multiple Resize')} Processing`)
